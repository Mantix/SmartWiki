...
abstract class SmartWikiModelElement {
	// SmartWikiString object
	private $searchPath;
	...
	// SmartWikiState object
	private $state;

	// SmartWikiString object
	private $packageID;

	// SmartWikiPackage object
	private $package;
	...
	function __construct() {
		$this->setState(SmartWikiState::LOG_NOACTION);
	}
	...
	public function getValuesFromArticle() {
		...
		if (($content_template_start !== false) && ($content_template_end !== false)) {
			...
			$this->setContent(new SmartWikiString(substr($content, 0, $content_template_start) . substr($content, $content_template_end)));
		} else {
			$content_template = '';
			$this->setContent(new SmartWikiString($content));
		}
	 ...
	 */
	abstract public static function parseXMI(SmartWikiXmlParser $xmlParser);
	...
	*
	* @var SmartWikiXmlParser $xmlParser - The array we need to search in
	*
	...
	*/
	public static function arraySearchRecursive($searchKey, $searchArray, $searchPath = '') {
		...
		if (is_array($searchArray)) {
			foreach ($searchArray AS $key => $value) {
				...
				$returnArray = array_merge($returnArray, SmartWikiModelElement::arraySearchRecursive($searchKey, $value, $innerPath));
			}
		}
	 ...
	 * 
	 * @var SmartWikiSyntaxTransformer $syntaxTransformer - The transformer
	 * 
	 * @var String $formName - The name of the form we need to use
	 * 
	 * @return $titleArray - An array of the Titles we've created
	 */
	public static function createPages(SmartWikiString $formName, array $elementArray, SmartWikiLog $log) {
		# Get the template of this page form
		$form_template = SmartWikiModelElement::getTemplatesOfForm($formName);
	...
	public function equals(SmartWikiModelElement $other) {
	...
	*/
	private static function getTemplatesOfForm(SmartWikiString $formName) {
	...
	public function __toString() {
		return 'SmartWikiModelElement: ' . $this->getID() . '; ' . $this->getTitle();
	}
	...
	*/
	public function getContainer($searchPath, SmartWikiXmlParser $xmlParser, $containerName = 'UML:Package') {
		# Do some magic
	...
	public function setSearchPath(SmartWikiString $searchPath) {
	...
	public function setId(SmartWikiString $id) {
		$this->id = $id;
	}
	...
	public function setState($state = SmartWikiState::LOG_UNKNOWN) {
		$this->state = $state;
	}
	...
	public function setPackageID(SmartWikiString $packageID) {
	...
	public function setPackage(SmartWikiPackage $package = NULL) {
	...
	public function setName(SmartWikiString $name) {
		$this->name = $name;
	}
	...
	public function setDescription(SmartWikiString $description) {
	...
	public function setContent(SmartWikiString $content) {
	...
	public function setOrder(SmartWikiNumber $order) {
		$this->order = $order;
	}
}
?>