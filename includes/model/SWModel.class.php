...
class SWModel {
	...
	public function getAttributeByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setAttribute(SWAttribute $attribute) {
		foreach ($this->attributes AS $attributeKey => $attributeValue) {
	...
	public function getEnumeration(Title $title = NULL) {
		...
		foreach ($this->enumerations AS $enumerationKey => $enumerationValue) {
			if ( get_class($enumerationValue) == "SWEnumeration" ) {
	...
	public function getEnumerationByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setEnumeration(SWEnumeration $enumeration) {
		foreach ($this->enumerations AS $enumerationKey => $enumerationValue) {
	...
	public function getAssociationByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setAssociation(SWAssociation $association) {
		foreach ($this->associations AS $associationKey => $associationValue) {
	...
	public function getAssociationClassByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setAssociationClass(SWAssociationClass $associationClass) {
		foreach ($this->associationClasses AS $associationClassKey => $associationClassValue) {
	...
	public function getGeneralizationByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setGeneralization(SWGeneralization $generalization) {
		foreach ($this->generalizations AS $generalizationKey => $generalizationValue) {
	...
	public function getClassByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setClass(SWClass $class) {
		foreach ($this->classes AS $classKey => $classValue) {
	...
	public function getPackageByID(SWString $id = NULL) {
		if ($id == NULL) {
			return NULL;
		}
	...
	public function setPackage(SWPackage $package) {
		foreach ($this->packages AS $packageKey => $packageValue) {
	...
	public function sortAndOrderNr() {
		...
		usort($this->classes, array($this, "modelSWClassSortFunction"));
		usort($this->attributes, array($this, "modelSWAttributeSortFunction"));
		usort($this->associations, array($this, "modelSWClassSortFunction"));
		usort($this->associationClasses, array($this, "modelSWClassSortFunction"));
		usort($this->generalizations, array($this, "modelSWClassSortFunction"));
	...
	private function setOrderNumbers() {
		...
		for ($y = 0; $y < count($this->packages); $y++) {
			...
			for ($i = 0; $i < count($this->classes); $i++) {
				...
				if ($class->getPackage() == null || $class->getPackage()->getTitle()->equals($packageTitle)) {
					$class->setOrder(new SWNumber($orderNr));
					$orderNr++;
				}
			}
		...
		foreach ($this->attributes AS $attributeKey => $attributeValue) {
			...
			$this->attributes[$attributeKey]->setOrder(new SWNumber($attributeOrderNrs[$attributeClassTitle]));
		}		
	}
	
	// Sort SmartWiki Classes on: package name, current order nr, title
	private function modelSWClassSortFunction($a,$b) {
	...
	private function modelSWAttributeSortFunction($a,$b) {
	...
	public function fillClassAssociations() {
		...
		foreach($classes as $keyClass => $valueClass) {
			...
			if ( isset($assoLinks[$className]) && isset($assoLinks[$className]['from']) && is_array($assoLinks[$className]['from']) ) {
				usort($assoLinks[$className]['from'], array("SWModel", "sortSWClass"));
			...
			if ( isset($assoLinks[$className]) && isset($assoLinks[$className]['to']) && is_array($assoLinks[$className]['to']) ) {
				usort($assoLinks[$className]['to'], array("SWModel", "sortSWClass"));
	...
	public static function sortSWClass($a, $b) {
	...
	static public function getFilledSWModel() {
