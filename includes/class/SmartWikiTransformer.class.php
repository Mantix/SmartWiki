...
class SmartWikiTransformer {
	...
	public function init() {
		# For logging
		$this->log = new SmartWikiLog(new SmartWikiString('transformation'));
		...
		while ($titleIterator->valid()) {
			...
			$this->log->add($titleObject, SmartWikiState::LOG_DELETED);
			$titleIterator->next();
		}
		...
		*/
		...
		$titleArray = $this->log->getTitlesWithState(SmartWikiState::LOG_DELETED);
	...
	private function createFilterPages() {
		...
		for($i=0; $i<count($this->associations);$i++) {
			...
			if ( trim($originalText) != trim($newText) ) {
				$this->log->add($title, ($title->isKnown() ? SmartWikiState::LOG_EDITED : SmartWikiState::LOG_CREATED));
			} else {
				$this->log->add($title, ($title->isKnown() ? SmartWikiState::LOG_NOACTION : SmartWikiState::LOG_CREATED));
			}
	...
	public function createSubmenus() {
		...
		$this->log->add($title, SmartWikiState::LOG_EDITED);
		$article = new Article($title);
		$newText = '';
...
# TODO			"** " . $wgSitename . " Manager | " . $wgSitename . " Manager\n" . 
	...
	*/ 			
				...
				$this->log->add($packageTitle, ($packageTitle->isKnown() ? SmartWikiState::LOG_EDITED : SmartWikiState::LOG_CREATED));
		...
		*/
		...
		$this->log->add($main_title, ($main_title->isKnown() ? SmartWikiState::LOG_EDITED : SmartWikiState::LOG_CREATED));
	...
	private function createClassCategoryPages() {
		for ($i = 0; $i < count($this->classes); $i++) {
			...
			$category_text = "";
			if ($this->classes[$i]->getAbstract()->equals(new SmartWikiBoolean(FALSE))) {
			...
			if ( trim($originalText) != trim($category_text) ) {
				$this->log->add($category_title, ($category_title->isKnown() ? SmartWikiState::LOG_EDITED : SmartWikiState::LOG_CREATED));
			} else {
				$this->log->add($category_title, ($category_title->isKnown() ? SmartWikiState::LOG_NOACTION : SmartWikiState::LOG_CREATED));
			}
			...
			$article_text = 
				...
				($this->classes[$i]->getAbstract()->equals(new SmartWikiBoolean(FALSE)) ? 
			...
			if ( trim($originalText) != trim($article_text) ) {
				$this->log->add($article_title, ($article_title->isKnown() ? SmartWikiState::LOG_EDITED : SmartWikiState::LOG_CREATED));
				
			} else {
				$this->log->add($article_title, ($article_title->isKnown() ? SmartWikiState::LOG_NOACTION : SmartWikiState::LOG_CREATED));
			}
	...
	private function createClassProperties() {
		for ($i = 0; $i < count($this->attributes); $i++) {
			...
			if ( trim($originalText) != trim($property_text) ) {
				$this->log->add($propertyTitle, ($propertyTitle->isKnown() ? SmartWikiState::LOG_EDITED : SmartWikiState::LOG_CREATED));
			} else {
				$this->log->add($propertyTitle, ($propertyTitle->isKnown() ? SmartWikiState::LOG_NOACTION : SmartWikiState::LOG_CREATED));
	...
	private function createClassTemplates() {
		# All the classes
		for ($i = 0; $i < count($this->classes); $i++) {
			$isKnowledgeElement = $this->classes[$i]->getIsKnowledgeElement()->equals(new SmartWikiBoolean(TRUE));
	...
	private function createClassTemplate(&$currentClass, $isInformationEntity = true, $isKnowledgeElement = true) {
		...
		$isInformationEntity = $currentClass->getAbstract() == NULL || $currentClass->getAbstract()->equals(new SmartWikiBoolean(FALSE));
		$isKnowledgeElement = $currentClass->getIsKnowledgeElement() != NULL && $currentClass->getIsKnowledgeElement()->equals(new SmartWikiBoolean(TRUE));
...
//				"|Completed=42\n" .  
		...
		$isAssociationClass = get_class($currentClass) == "SmartWikiAssociationClass" ? true : false;
...
/*
		...
		*/
		...
		if ( get_class($currentClass) == "SmartWikiAssociationClass" ) {
			$targetClass = $currentClass->getToClass();
			$fromClass = $currentClass->getFromClass();
			$d = "";
			if ( $_SERVER['SITE_ENV'] == 'D' ) $d = "[SmartWikiAssociationClass From=".$fromClass->getName()." To=".$targetClass->getName()." Name=".$currentClass->getName()."]";
			$template_text .=
		...
		foreach($this->associationClasses as $key => $value) {
			if ( $value->getToClass()->getTitle()->equals($currentClass->getTitle()) ) {
				...
				//if ( $_SERVER['SITE_ENV'] == 'D' ) $d = "[SmartWikiAssociationClass (reverseName) From=".$fromClass->getName()." To=".$toClass->getName()." Name=".$value->getName()."]";
				$template_text .=
					"<tr>" .
		...
		if ( get_class($currentClass) == "SmartWikiAssociationClass" ) {
			if ( $currentClass->getCanHaveDerivables() != null && $currentClass->getCanHaveDerivables()->equals(new SmartWikiBoolean(true) ) ) {
				$template_text .= self::readTemplate("SmartWiki_Derivable".($isAssociationClass?"":"_AC"));
			}
			if ( $currentClass->getCanHaveSubelements() != null && $currentClass->getCanHaveSubelements()->equals(new SmartWikiBoolean(true) ) ) {
		...
		$template_text .= 
			...
			"</div>" . (get_class($currentClass) == "SmartWikiAssociationClass" ? "" :"" ) .
	...
	private function createClassForms() {
		for ($i = 0; $i < count($this->classes); $i++) {
			...
			if ($this->classes[$i]->getAbstract()->equals(new SmartWikiBoolean(FALSE))) {
				...
				$isInformationEntity = $this->classes[$i]->getAbstract()->equals(new SmartWikiBoolean(FALSE));
				$isKnowledgeElement = $this->classes[$i]->getIsKnowledgeElement()->equals(new SmartWikiBoolean(TRUE));
				...
				*/
				...
				if ($this->classes[$i]->getCanHaveDerivables()->equals(new SmartWikiBoolean(TRUE))) {
					$form_text .=
				...
				if ($this->classes[$i]->getCanHaveSubelements()->equals(new SmartWikiBoolean(TRUE))) {
					$form_text .=
	...
	private function createFormUsingClass($class, $allowMultiple = false, $isInformationEntity = false, $isKnowledgeElement = false, &$skipFields = array(), $rootClass = null) {
		...
		if ( get_class($class) == "SmartWikiAssociationClass" ) {
		...
		if ( $thisClassIsAnAssociationClass ) {
			// Add the derivable option is needed
			if ( $class->getCanHaveDerivables() != null && $class->getCanHaveDerivables()->equals(new SmartWikiBoolean(TRUE))) {
			...
			// Add the subelement option is needed
			if ( $class->getCanHaveSubelements() != null && $class->getCanHaveSubelements()->equals(new SmartWikiBoolean(TRUE))) {
	...
	public static function getFieldsByCategory($categoryName) {
		...
		while ($titleIterator->valid()) {
			$titleObject = $titleIterator->current();
			$f = SmartWikiTransformer::getFieldsFromPage($titleObject);
			$fields[] = $f;
			$titleIterator->next();
		}
