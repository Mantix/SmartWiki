...
class SmartWikiInitialize {
	...
	/**
	 * Initialize the SmartWiki links in the menu
	 * 
	 ...
	 */
	public function initializeMenu(SmartWikiLog $log, $showInMenu = true, $links_category = array()) {
		...
		// Do the edit
		$logStatus = SmartWikiPage::editPage($article, $newText, '(Re)created the menu for initialization of SmartWiki.');
		// Add this to the log
	 ...
	 */
	public function initializeFile(SmartWikiLog $log) {
	 ...
	 */
	public function initializeNamespace(SmartWikiLog $log, $namespace_id) {
		...
		if ( MWNamespace::exists( $namespace_id ) || ($namespace_id == NS_MAIN)) {
			...
			for ($i = 0; $i < $file_count; $i++) {
				...
				$file_contents = SmartWiki::readFile($file_list[$i]);
				...
				$logStatus = SmartWikiPage::editPage($article, $file_contents, wfMsgForContent('smartwiki-initialize-recreate', $namespace_name, $name));
